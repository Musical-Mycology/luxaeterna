"""Lux Aeterna — TickPacer: pace a loop to absolute deadlines.

A loop that does its work and then sleeps the rest of its interval runs at
...
so a 44 Hz output loop ran ~37 Hz. See
docs/superpowers/specs/2026-09-25-output-loop-deadline-pacing-design.md.
...
per-tick jitter (~4 ms on macOS) the platform's sleep adds.
